...
-   `mca_files`: contains also the figures generated by this notebook.
    This directory is auto-generated by RStudio when this notebook was
    knitted into a regular markdown file.
...
command `install.packages("packageName")`. RStudio will also notice you
...
The notebook can be run such that the properties of the data set are
plotted in an interactive Leaflet 2D map. However, the interactive map
cannot be used when knitting the notebook into a regular markdown file.
Therefore, `use_interactive_map` is set to `FALSE` for generating this
markdown file but can be set to `TRUE` when running the notebook in
RStudio.
...
which makes it a non-robust method in that sense. Let’s use KNN with
...
and `Lon` anymore now that the KNN algorithm has regrouped the
